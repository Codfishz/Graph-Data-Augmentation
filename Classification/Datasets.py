...
#        g.add_edges(src, dst)
#        g.add_edges(dst, src)
        g = dgl.graph((src, dst), num_nodes=1284)
#        g.add_edges(dst, src)
        ...
        self.labels=torch.from_numpy(Y_label_num)
        self.edges=edges1k
        self.cthink=torch.from_numpy(train_data_cthick)
...
#        self.sampleGraph=nx_G
    ...
    def __len__(self):
        return len(self.labels)
    def __getitem__(self, idx):
        with self.sampleGraph.local_scope:
